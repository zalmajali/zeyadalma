...
@Component({
  selector: 'app-equipmentadd',
  templateUrl: './equipmentadd.component.html',
  styleUrls: ['./equipmentadd.component.scss'],
})
export class EquipmentaddComponent  implements OnInit {
  ...
  public add_equipments:any;
  public equipments_name_add_succ:any;
  public equipments_name_add_failed:any;
  public equipments_name_add_failed_data:any;
//mat name
  ...
  initialiseTranslation(){
    ...
    this.translate.get('equipments_add').subscribe((res: string) => {
      this.pageTitle = res;
    });
    ...
    this.translate.get('add_equipments').subscribe((res: string) => {
      this.add_equipments = res;
    });
    this.translate.get('equipments_name_add_succ').subscribe((res: string) => {
      this.equipments_name_add_succ = res;
    });
    this.translate.get('equipments_name_add_failed').subscribe((res: string) => {
      this.equipments_name_add_failed = res;
    });
    this.translate.get('equipments_name_add_failed_data').subscribe((res: string) => {
      this.equipments_name_add_failed_data = res;
    });
  }
  async ngOnInit() {
  ...
  functionAddNewEquipments() {
    ...
    let sendValues = {'zoneId':this.zoneId,'eventId':this.eventId,'inspId':this.inspId,'itemId':this.itemName,'quantity':this.quantity,'notes':this.notes};
    this.inspectionpointsService.addEquipment(sendValues).then(async data=>{
      this.returnAddEquipmentsResultData = data;
      let errorData = this.returnAddEquipmentsResultData.Error.ErrorCode;
      if(errorData == 1){
        this.displayResult(this.equipments_name_add_succ);
        this.modalController.dismiss({
          "key":1
        })
      }else if(errorData == 2){
        this.displayResult(this.equipments_name_add_failed);
      }else if(errorData == 3 || errorData == 4 || errorData == 5){
        this.displayResult(this.equipments_name_add_failed_data);
      }
    });
    this.isdisabled = true;
    return true;
