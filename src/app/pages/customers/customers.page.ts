...
export class CustomersPage implements OnInit {
  ...
  public customers_options_one: any;
  public customers_options_tow: any;
  public customers_call_msg: any;
  public customers_loc_msg_one: any;
  public customers_loc_msg_tow: any;
  //check login
  public fullName:any;
  public userId:any;
  public username:any;
  ...
  initialiseTranslation(){
    ...
    this.translate.get('customers_title').subscribe((res: string) => {
      this.pageTitle = res;
    });
    this.translate.get('customers_options_one').subscribe((res: string) => {
      this.customers_options_one = res;
    });
    this.translate.get('customers_options_tow').subscribe((res: string) => {
      this.customers_options_tow = res;
    });
    this.translate.get('customers_call_msg').subscribe((res: string) => {
      this.customers_call_msg = res;
    });
    this.translate.get('customers_loc_msg_one').subscribe((res: string) => {
      this.customers_loc_msg_one = res;
    });
    this.translate.get('customers_loc_msg_tow').subscribe((res: string) => {
      this.customers_loc_msg_tow = res;
    });
  }
  async ngOnInit() {
  ...
  functionCallNumber(numer:any){
    this.callNumber.callNumber(numer, true)
      ...
      .catch(err =>{
        this.displayResult(this.customers_call_msg);
      });
  }
  functionGoLocation(location:any){
    /*this.geolocation.getCurrentPosition().then(position=>{
      ...
      this.launchNavigator.navigate([res[0], res[1]],options)
        .then(success =>{
          this.displayResult(this.customers_loc_msg_tow);
        },error=>{
          this.displayResult(this.customers_loc_msg_one);
        })
    }).catch(e=>{
      this.displayResult(this.customers_loc_msg_one);
    })*/
