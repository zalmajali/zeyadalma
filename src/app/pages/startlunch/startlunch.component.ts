...
@Component({
  selector: 'app-startlunch',
  templateUrl: './startlunch.component.html',
  styleUrls: ['./startlunch.component.scss'],
})
export class StartlunchComponent  implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.selectedTime = dateUsed.getHours().toString().padStart(2, '0')+':'+dateUsed.getMinutes().toString().padStart(2, '0')+':'+'00'//2024-02-19T01:36:00
  }
  selectDate(event:any){
    if(event.target.value!=undefined){
      this.selectTime = event.target.value;
      let checkVal = this.selectTime.split('T');
      this.selectedTime = checkVal[1];
    }
    this.modalController.dismiss({
