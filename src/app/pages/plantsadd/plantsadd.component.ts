...
@Component({
  selector: 'app-plantsadd',
  templateUrl: './plantsadd.component.html',
  styleUrls: ['./plantsadd.component.scss'],
})
export class PlantsaddComponent  implements OnInit {
  ...
  public plants_name_add_succ:any;
  public plants_name_add_failed:any;
  public plants_name_add_failed_data:any;
  //mat name
  public errorPlantsName: any="";
  public plantsName: any;
  ...
  initialiseTranslation(){
    ...
    this.translate.get('plants_add').subscribe((res: string) => {
      this.pageTitle = res;
    });
    ...
    this.translate.get('add_plants').subscribe((res: string) => {
      this.add_plants = res;
    });
    this.translate.get('plants_name_add_succ').subscribe((res: string) => {
      this.plants_name_add_succ = res;
    });
    this.translate.get('plants_name_add_failed').subscribe((res: string) => {
      this.plants_name_add_failed = res;
    });
    this.translate.get('plants_name_add_failed_data').subscribe((res: string) => {
      this.plants_name_add_failed_data = res;
    });
  }
  async ngOnInit() {
  ...
  functionAddNewPlants() {
    ...
    let sendValues = {'zoneId':this.zoneId,'eventId':this.eventId,'inspId':this.inspId,'plantsId':this.plantsName,'amount':this.amount};
    this.inspectionpointsService.addPlants(sendValues).then(async data=>{
      this.returnAddPlantsResultData = data;
      let errorData = this.returnAddPlantsResultData.Error.ErrorCode;
      if(errorData == 1){
        this.displayResult(this.plants_name_add_succ);
        this.modalController.dismiss({
          "key":1
        })
      }else if(errorData == 2){
        this.displayResult(this.plants_name_add_failed);
      }else if(errorData == 3 || errorData == 4 || errorData == 5){
        this.displayResult(this.plants_name_add_failed_data);
      }
    });
    this.isdisabled = true;
    return true;
